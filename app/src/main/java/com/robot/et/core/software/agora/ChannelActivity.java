...
public class ChannelActivity extends BaseEngineEventHandlerActivity {
	...
	public void onCreate(Bundle savedInstance) {
		...
		switch (mCallingType) {
		case DataConfig.JPUSH_CALL_VIDEO:// 视频
			DataConfig.isAgoraVideo = false;
			DataConfig.isAlarmTips = false;
			video();
			
			break;
		case DataConfig.JPUSH_CALL_VOICE:// 语音
			DataConfig.isAgoraVoice = false;
			DataConfig.isAlarmTips = false;
			voice();
			
			break;
		case DataConfig.JPUSH_CALL_LOOK:// 查看
			isLook = true;
			...
			new Handler().postDelayed(new Runnable() {
				...
				public void run() {
					if(!isUserJoined){
						...
						BroadcastShare.textToSpeak(DataConfig.TYPE_AGORA_HANGUP, content);
	...
	BroadcastReceiver receiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equals(BroadcastAction.ACTION_CLOSE_AGORA)){//关闭agora
				...
				if(currentType == DataConfig.JPUSH_CALL_VIDEO){//当前是视频通话
					if(isNetBad){//网络差
						if(isCurrentVideo){
	...
	//语音通话
	private void voice() {
		mCallingType = DataConfig.JPUSH_CALL_VOICE;
		...
		new Handler().postDelayed(new Runnable() {
			@Override
			public void run() {
				updateRemoteUserViews(DataConfig.JPUSH_CALL_VOICE);
			}
		}, 500);
	...
	private void video() {
		...
		if (DataConfig.JPUSH_CALL_VIDEO == mCallingType) { // 视频
			rtcEngine.setEnableSpeakerphone(true);// 扬声器开
			rtcEngine.muteAllRemoteAudioStreams(false);
		} else if (DataConfig.JPUSH_CALL_LOOK == mCallingType) { // 查看
			rtcEngine.setEnableSpeakerphone(false);// 扬声器关
			rtcEngine.muteAllRemoteAudioStreams(true);//静音所有远端音频 
		}
		
		mCallingType = DataConfig.JPUSH_CALL_VIDEO;
		...
		new Handler().postDelayed(new Runnable() {
			@Override
			public void run() {
				updateRemoteUserViews(DataConfig.JPUSH_CALL_VIDEO);
			}
		}, 500);
	...
	private void updateRemoteUserViews(int callingType) {
		...
		if (DataConfig.JPUSH_CALL_VIDEO == callingType) {
			visibility = View.GONE;
		} else if (DataConfig.JPUSH_CALL_VOICE == callingType) {
	...
	public synchronized void onFirstRemoteVideoDecoded(final int uid,int width, int height, final int elapsed) {
		...
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				if (remoteUserView != null && DataConfig.JPUSH_CALL_VIDEO == mCallingType) {
		...
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				remoteView.findViewById(R.id.remote_user_voice_container)
						.setVisibility((DataConfig.JPUSH_CALL_VOICE == mCallingType || (DataConfig.JPUSH_CALL_VIDEO == mCallingType && muted)) ? View.VISIBLE : View.GONE);
				remoteView.invalidate();
			}
		});
