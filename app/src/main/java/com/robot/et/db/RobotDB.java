...
public class RobotDB {
	...
	public synchronized static RobotDB getInstance(){
		if(instance == null){
	...
	public void deleteAllRemindInfo(){
		String sql = "delete from reminds";
		SQLiteDatabase db = helper.getWritableDatabase();
	...
	//新增用户联系方式
	public void addUserPhoneInfo(UserInfo info){
		String sql = "insert into users(userId,userName,userPhone) values(?,?,?)";
		SQLiteDatabase db = helper.getWritableDatabase();
		db.execSQL(sql, new String[]{String.valueOf(info.getUserId()),info.getUserName(),info.getUserPhone()});
		db.close();
	}
	
	//获取所有用户联系表
	public List<UserInfo> getAllUserPhoneInfos() {
		...
		while (c.moveToNext()) {
			...
			info.setUserId(c.getInt(c.getColumnIndex("userId")));
			info.setUserName(c.getString(c.getColumnIndex("userName")));
			info.setUserPhone(c.getString(c.getColumnIndex("userPhone")));
			infos.add(info);
		}
		c.close();
		db.close();
	...
	//删除用户联系方式
	public void deleteAllUserPhoneInfos(){
	...
	public void addScript(ScriptInfo info){
		String sql = "insert into script(userPhone,robotNum,scriptContent,scriptType,remarks) values(?,?,?,?,?)";
		SQLiteDatabase db = helper.getWritableDatabase();
		db.execSQL(sql, new String[]{info.getUserPhone(),info.getRobotNum(),info.getScriptContent(),String.valueOf(info.getScriptType()),info.getRemarks()});
		db.close();
	}
	...
	public void addScriptAction(ScriptActionInfo info){
		String sql = "insert into scriptAction(scriptId,actionType,content,spareContent,direction,remarks) values(?,?,?,?,?,?)";
		SQLiteDatabase db = helper.getWritableDatabase();
		db.execSQL(sql, new String[]{String.valueOf(info.getScriptId()),String.valueOf(info.getActionType()),info.getContent(),info.getSpareContent(),
				String.valueOf(info.getDirection()),info.getRemarks()});
		db.close();
	}
	...
	public List<ScriptActionInfo> getScriptActionInfos(int scriptId){
		...
		while (c.moveToNext()) {
			...
			info.setDirection(c.getInt(c.getColumnIndex("direction")));
			info.setRemarks(c.getString(c.getColumnIndex("remarks")));
			infos.add(info);
		}
		c.close();
		db.close();
