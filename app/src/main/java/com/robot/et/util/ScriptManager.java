...
public class ScriptManager {
        ...
        if(infos != null && infos.size() > 0){
            ...
            switch (info.getActionType()){
                ...
                case ScriptConfig.SCRIPT_DIRECTION_BACK://向后走
                    ...
                    break;
                case ScriptConfig.SCRIPT_DIRECTION_LEFT://向左走
                    Log.i("netty", "doScriptAction() 向左走");
                    BroadcastShare.controlMove(String.valueOf(ControlMoveConfig.MOVE_DIRECTION_LEFT),info.getContent());
                    setNewScriptInfos(infos,true);

                    break;
                case ScriptConfig.SCRIPT_DIRECTION_RIGHT://向右走
                    Log.i("netty", "doScriptAction() 向右走");
    ...
    //更新剧本的内容
    public static void setNewScriptInfos(List<ScriptActionInfo> infos,boolean isResume){
        if(infos != null && infos.size() > 0){
            infos.remove(0);
